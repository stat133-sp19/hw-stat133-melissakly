...
library(dplyr)
gsw_shots = read.csv("../data/shots-data.csv")
points2 = summarise(group_by(gsw_shots, name),
                  total = sum(shot_type == "2PT Field Goal"),
                  made = sum(shot_type == "2PT Field Goal"&shot_made_flag == "shot_yes"))

points2$perc_made = points2$made / points2$total

points2 = points2 %>% arrange(desc(perc_made))

points3 = summarise(group_by(gsw_shots, name),
                  total = sum(shot_type == "3PT Field Goal"),
                  made = sum(shot_type == "3PT Field Goal"&shot_made_flag == "shot_yes"))

points3$perc_made = points3$made / points3$total

points3 = points3 %>% arrange(desc(perc_made))

overall <- summarise(group_by(gsw_shots, name),
                  total = sum(shot_type == "2PT Field Goal"|shot_type == "3PT Field Goal"),
                  made = sum(shot_made_flag == "shot_yes"))
overall$perc_made = overall$made / overall$total
overall = overall %>% arrange(desc(perc_made))
overall
```
...
- perc_made: the percentage of total shots made by each player. 

Points2 table is the Effective Shooting Percentage for 2 Point Field Goals for each player
Points3 table is the Effective Shooting Percentage for 3 Point Field Goals for each player
Overall table is the Effective Shooting Percentage for all types of Field Goals for each player.
